...
import { DEFAULT_TOP_SONGS_LIMIT, DEFAULT_ARTISTS_LIMIT, DEFAULT_ALBUMS_LIMIT } from '@/constants/musicConstants';
...
export function useCachedMusicData(limit: number = DEFAULT_TOP_SONGS_LIMIT) {
  ...
  const { lastFetched, cacheExpiry, loading, error } = useAppSelector(state => state.music);
...
export function useCachedSongsByGenre(genreId: number | null) {
  ...
  const songs = genreId ? getCachedData(songsByGenre[genreId], cacheExpiry) : undefined;
...
export function useCachedSongsByAlbum(albumId: number | null) {
  ...
  const songs = albumId ? getCachedData(songsByAlbum[albumId], cacheExpiry) : undefined;
...
export function useCachedSongsByArtist(artistId: number | null) {
  ...
  const songs = artistId ? getCachedData(songsByArtist[artistId], cacheExpiry) : undefined;
...
export function useMusicCacheStats() {
  ...
  return {
    ...
    artistsCacheValid: isCacheValid(state.allArtists.lastFetched, state.cacheExpiry),
    albumsCacheValid: isCacheValid(state.allAlbums.lastFetched, state.cacheExpiry),
